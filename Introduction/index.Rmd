...
1. Tricks to produce __html, pdf and docx__ outputs from one source file of __R Markdown__. If you learn R Markdown, you are likely to burn all our \(\LaTeX\) distributions and other authoring (typesetting) applications.

1. Step by step guide to develop an R package that can integrate Eviews R, and R Markdown.

1. Learn how to work with version control system, specifically [github](https://www.github.com). Therefore, you longer have to be saving your files with various names (such as update1, update2,final version, final version2 and so on)
...
However it will take few seconds for everyone, depending on computer speed, to __reproduce__ the map if s/he uses the following sets of __code__.
...
What do you think if you get some corrections/suggestions to modify the color legend of some or all of the countries on the map of Figure \@ref(fig:ECOWAS1)
, for example:
...
To some extent, \(\LaTeX\),  __Microsoft Office Word__, __LibreOffice__ and  __OpenOffice__ can make reports __dynamic and reproducible__. However, their capacity is limited as they make no provisions for `inline codes`.
